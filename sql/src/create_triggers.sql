...
DROP FUNCTION IF EXISTS addMessageId();
...
DROP TRIGGER IF EXISTS insertMessageId ON MESSAGE;
...
-- PROCEDURES FOR MESSAGES TABLE

CREATE OR REPLACE FUNCTION addMessageId () 
RETURNS "trigger" AS
$BODY$
BEGIN
...
CREATE TRIGGER insertMessageId BEFORE INSERT ON MESSAGE
	FOR EACH ROW EXECUTE PROCEDURE addMessageId();
